...
SYSTEM_PROMPT = (
    ...
    "If the message instead reveals a B2B billing dispute (e.g. 'we already "
    "paid this', 'the amount is wrong') rather than a PTP or FAQ intent, call "
    "flag_b2b_dispute with the raw message and your extracted reason - you "
    "don't own invoice data, so never try to resolve or comment on the "
    "dispute's validity yourself, just hand it off. If the customer is "
    "instead asking you to resend or regenerate a payment link ('send that "
    "link again', 'I lost the link'), call flag_payment_link_resend_request "
    "- you don't own checkout state either, so hand this off the same way "
    "rather than trying to find or fabricate a link yourself. "
