...
def start_sequence(subscription_id: str, amount: int, customer_name: str,
    ...
    schedule_dunning_sequence(subscription_id, link["id"])
...
def evaluate_next_touch(subscription_id: str, stage: int) -> dict:
    ...
    a date. Otherwise returns the template + link to send via the two-hop
    delegation to the Conversational NLP Agent - this tool does not send
    anything itself, Srv8 is exclusively that agent's."""
    r = get_redis()
    ...
    return {
        ...
        "dunning_link_id": subscription["dunning_link_id"],
    }
